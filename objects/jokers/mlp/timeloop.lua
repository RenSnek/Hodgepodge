...
SMODS.Joker {
    ...
    cost = 20,
    calculate = function(self,card,context)
        if context.destroy_card and context.cardarea == G.play and ((G.GAME.chips + SMODS.calculate_round_score()) >= G.GAME.blind.chips) and G.GAME.blind.boss then
            return {
                remove = (context.destroy_card == context.destroying_card)
            }
        end
    end,
    add_to_deck = function(self,card,from_debuff)
        G.GAME.hodge_use_scoring_ante = G.GAME.hodge_use_scoring_ante + 1
        if card.ability.extra.first_add then
            G.GAME.hodge_scoring_ante = G.GAME.round_resets.ante
        end
        card.ability.extra.first_add = false
    end,
